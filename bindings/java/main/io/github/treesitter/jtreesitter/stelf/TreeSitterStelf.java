package io.github.treesitter.jtreesitter.stelf;

import java.lang.foreign.*;

public final class TreeSitterStelf {
    ...
    private static final TreeSitterStelf INSTANCE = new TreeSitterStelf();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterStelf() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_stelf");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-stelf");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-stelf");
